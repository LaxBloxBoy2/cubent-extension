...
build_extension() {
  echo "🔨 Building the qapt coder extension..."
  cd ..
  mkdir -p bin
  pnpm build -- --out ../bin/qapt-coder-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension bin/qapt-coder-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "qaptcoder.qapt-coder"; then
  code --install-extension qaptcoder.qapt-coder &>/dev/null || exit 1
fi
