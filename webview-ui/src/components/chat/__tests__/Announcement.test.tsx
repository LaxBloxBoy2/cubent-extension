	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 cubent Code ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `cubent Code ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 cubent Code ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`cubent Code ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
