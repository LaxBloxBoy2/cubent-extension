...
- Fix menu breaking when cubent is moved between primary and secondary sidebars (thanks @chrarnoldus!)
...
- Revert "Switch to the new cubent message parser" (appears to cause a tool parsing bug)
...
- Switch to the new cubent message parser for improved performance (thanks Cline!)
...
- Add IPC types to cubent-code.d.ts
...
- You can now toggle the `terminal.integrated.inheritEnv` VSCode setting directly for the cubent coder settings (thanks @KJ7LNW)
...
- Fix a nasty bug that would cause cubent coder to hang, particularly in orchestrator mode
...
- Simplify and streamline cubent coder's quick actions
- Allow cubent coder settings to be imported from the welcome screen (thanks @julionav)
...
- Fix word wrapping in cubent message title (thanks @zhangtony239!)
...
- Focus cubent Code in the sidebar when running tasks in the sidebar via the API
- Improve subtasks UI

## [3.11.10] - 2025-04-08

- Fix bug where nested .cubent/rules directories are not respected properly (thanks @taisukeoe!)
...
- You can now place multiple rules files in the .cubent/rules/ and .cubent/rules-{mode}/ folders (thanks @upamune!)
...
- Add command to focus cubent Code input field (thanks @axkirillov!)
...
- Allow processes to access the cubent Code API via a unix socket
...
- Support project-level MCP config in .cubent/mcp.json (thanks @aheizi!)
...
- Add an activation command that other extensions can use to interface with cubent Code (thanks @gtaylor!)
...
- Pass current language to system prompt correctly so cubent thinks and speaks in the selected language

## [3.9.0] - 2025-03-18

- Internationalize cubent Code into Catalan, German, Spanish, French, Hindi, Italian, Japanese, Korean, Polish, Portuguese, Turkish, Vietnamese, Simplified Chinese, and Traditional Chinese (thanks @feifei325!)
- Bring back support for MCP over SSE (thanks @aheizi!)
- Add a text-to-speech option to have cubent talk to you as it works (thanks @heyseth!)
...
- Fix wheel scrolling when cubent is opened in editor tabs (thanks @GitlyHallows!)
...
- Add an option in the prompts tab to save tokens by disabling the ability to ask cubent to create/edit custom modes for you (thanks @hannesrudolph!)
...
- Add opt-in telemetry to help us improve cubent Code faster (thanks Cline!)
...
- Add support for a .rooignore to prevent cubent Code from read/writing certain files, with a setting to also exclude them from search/lists (thanks Cline!)
...
- Support running cubent in multiple editor windows simultaneously (thanks @samhvw8!)
...
- Add a new “Human Relay” provider that allows you to manually copy information to a Web AI when needed, and then paste the AI's response back into cubent Code (thanks @NyxJae)!
...
- Fix context window truncation math (see [#1173](https://github.com/RooCodeInc/cubent-Code/issues/1173))
...
- Introducing cubent Code 3.7, with support for the new Claude Sonnet 3.7. Because who cares about skipping version numbers anymore? Thanks @lupuletic and @cte for the PRs!
...
- Fix system prompt to make sure cubent knows about all available modes
...
- Prompt tweaks to make cubent better at creating new custom modes for you

## [3.3.6]

- Add a "new task" tool that allows cubent to start new tasks with an initial message and mode
...
- New cubent Code community Discord at https://roocode.com/discord!
...
- **Name Change From cubent Cline to cubent Code:** We're excited to announce our new name! After growing beyond 50,000 installations, we've rebranded from cubent Cline to cubent Code to better reflect our identity as we chart our own course.

- **Custom Modes:** Create your own personas for cubent Code! While our built-in modes (Code, Architect, Ask) are still here, you can now shape entirely new ones:
...
- This release adds chat modes! Now you can ask cubent Code questions about system architecture or the codebase without immediately jumping into writing code. You can even assign different API configuration profiles to each mode if you prefer to use different models for thinking vs coding. Would love feedback in the new cubent Code Reddit! https://www.reddit.com/r/RooCode
...
- Incorporate Premshay's [PR](https://github.com/RooCodeInc/cubent-Code/pull/60) to add support for Amazon Nova and Meta Llama Models via Bedrock (3, 3.1, 3.2) and unified Bedrock calls using BedrockClient and Bedrock Runtime API
...
- Incorporate dbasclpy's [PR](https://github.com/RooCodeInc/cubent-Code/pull/54) to add support for gemini-exp-1206
...
- Fix https://github.com/RooCodeInc/cubent-Code/issues/50 where sound effects were not respecting settings
...
- Incorporate lloydchang's [PR](https://github.com/RooCodeInc/cubent-Code/pull/42) to add support for OpenRouter compression
...
- cubent Cline now allows configuration of which commands are allowed without approval!
...
- cubent Cline now runs in all VSCode-compatible editors
...
- cubent Cline now can run side-by-side with Cline

## [2.1.3]

- cubent Cline now allows browser actions without approval when `alwaysAllowBrowser` is true
