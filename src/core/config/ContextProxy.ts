...
export class ContextProxy {
	...
	/**
	 * Refreshes the in-memory cache from VS Code storage
	 * Used for cross-instance synchronization
	 */
	public async refreshCache() {
		// Refresh global state cache
		for (const key of GLOBAL_STATE_KEYS) {
			try {
				this.stateCache[key] = this.originalContext.globalState.get(key)
			} catch (error) {
				logger.error(`Error refreshing global ${key}: ${error instanceof Error ? error.message : String(error)}`)
			}
		}

		// Refresh secret state cache
		const promises = SECRET_STATE_KEYS.map(async (key) => {
			try {
				this.secretCache[key] = await this.originalContext.secrets.get(key)
			} catch (error) {
