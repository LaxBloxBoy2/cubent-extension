...
import type { ToolName, ClineAsk, ToolProgressStatus } from "@cubent/types"
...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use":
			...
			// Check if tools should be restricted (first prompt or simple acknowledgment)
			const isFirstPrompt = cline.apiConversationHistory.length === 0
			const lastUserMessage = cline.apiConversationHistory[cline.apiConversationHistory.length - 1]
			const userMessageText = typeof lastUserMessage?.content === 'string'
				? lastUserMessage.content
				: Array.isArray(lastUserMessage?.content)
					? lastUserMessage.content.find(c => c.type === 'text')?.text || ''
					: ''
			const isSimpleAcknowledgment = cline.isSimpleAcknowledgment(userMessageText)

			if (isFirstPrompt || isSimpleAcknowledgment) {
				cline.consecutiveMistakeCount++
				pushToolResult(formatResponse.toolError("Tools are not allowed for simple messages. Please respond with text only."))
				break
			}
