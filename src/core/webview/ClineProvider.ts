	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider
{
	...
	constructor(
		...
		private readonly initialTab?: string,
	) {
		super()
		...
		// Listen for cross-instance state changes
		...
		try {
			this.userManagement = new UserManagementIntegration(this.context)
		} catch (error) {
			this.log(`User management not initialized (this is normal if not configured): ${error}`)
		}
	}
	...
	*/
	...
	/**
	 * Sets up cross-instance synchronization for state changes
	 */
	private setupCrossInstanceSync() {
		// No additional setup needed for now, but this method can be extended
		// if we need to listen to VS Code events or set up other synchronization mechanisms
	}
	...
	private async handleCrossInstanceStateChange(changeType: "configProfile" | "settings", data?: any) {
		try {
			...
			this.log(`Error handling cross-instance state change: ${error}`)
		}
	}

	/**
	 * Synchronizes this instance's state with another provider instance
	 * Used when opening settings in a new tab to ensure it has the latest state
	 */
	public async syncFromProvider(sourceProvider: ClineProvider) {
		try {
			// Force refresh the context proxy cache from VS Code storage
			await this.contextProxy.refreshCache()

			// Also refresh the source provider's cache to ensure we get the latest state
			await sourceProvider.contextProxy.refreshCache()

			// Post the refreshed state to our webview
			await this.postStateToWebview()

			this.log("Successfully synced state from source provider")
		} catch (error) {
			this.log(`Error syncing from source provider: ${error}`)
		}
	}
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		webviewView.webview.html =
			this.contextProxy.extensionMode === vscode.ExtensionMode.Development
				...
				: this.getHtmlContent(webviewView.webview, false)
	...
	public async postMessageToWebview(message: ExtensionMessage) {
		...
		// Emit the message for external listeners (like settings tabs)
		this.emit("webviewMessage", message)
	}

	public async handleWebviewMessage(message: any) {
		// Handle messages from external webviews (like settings tabs)
		const { webviewMessageHandler } = await import("./webviewMessageHandler")
		return webviewMessageHandler(this, message)
	}

	public async getWebviewContent(webview: vscode.Webview, settingsOnly: boolean = false): Promise<string> {
		// Public method to get webview content for external use
		return this.contextProxy.extensionMode === vscode.ExtensionMode.Development
			? await this.getHMRHtmlContent(webview)
			: this.getHtmlContent(webview, settingsOnly)
	}
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		try {
			...
			return this.getHtmlContent(webview, false)
		}
			...
			<html lang="en">
				<head>
					...
					<script nonce="${nonce}">
						...
						window.INITIAL_TAB = "${this.initialTab || ""}"
					</script>
	 ...
	 */
	private getHtmlContent(webview: vscode.Webview, settingsOnly: boolean = false): string {
	...
	public async handleModeSwitch(newMode: Mode) {
		...
		// Handle auto-approval for agent-auto mode
		if (newMode === "agent-auto") {
			// Enable all auto-approval settings for agent-auto mode (AUTOMATIC)
			await this.updateGlobalState("autoApprovalEnabled", true)
			await this.updateGlobalState("alwaysAllowReadOnly", true)
			await this.updateGlobalState("alwaysAllowWrite", true)
			await this.updateGlobalState("alwaysAllowExecute", true)
			await this.updateGlobalState("alwaysAllowBrowser", true)
			await this.updateGlobalState("alwaysAllowMcp", true)
			await this.updateGlobalState("alwaysAllowModeSwitch", true)
			await this.updateGlobalState("alwaysAllowSubtasks", true)
			await this.updateGlobalState("alwaysApproveResubmit", true)
		} else {
			// For all other modes, disable auto-approval (MANUAL)
			await this.updateGlobalState("autoApprovalEnabled", false)
			await this.updateGlobalState("alwaysAllowReadOnly", false)
			await this.updateGlobalState("alwaysAllowWrite", false)
			await this.updateGlobalState("alwaysAllowExecute", false)
			await this.updateGlobalState("alwaysAllowBrowser", false)
			await this.updateGlobalState("alwaysAllowMcp", false)
			await this.updateGlobalState("alwaysAllowModeSwitch", false)
			await this.updateGlobalState("alwaysAllowSubtasks", false)
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\cubent-Code\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "cubent-Code", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Cline/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Cline", "MCP")
		} else {
			// Linux: ~/.local/share/Cline/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "cubent-Code", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".cubent-code", "mcp")
		}
	...
	async renameTaskWithId(id: string, newTitle: string) {
		const taskHistory = this.getGlobalState("taskHistory") ?? []
		const taskIndex = taskHistory.findIndex((task) => task.id === id)
	...
	async togglePinTaskWithId(id: string, pinned: boolean) {
		const taskHistory = this.getGlobalState("taskHistory") ?? []
		const taskIndex = taskHistory.findIndex((task) => task.id === id)
	...
	// Add public getter for user management
	public getUserManagement(): UserManagementIntegration | undefined {
