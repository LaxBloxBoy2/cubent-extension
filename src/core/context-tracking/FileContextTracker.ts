...
// If a user modifies a file outside of cubent, the context may become stale and need to be updated.
// We do not want cubent to reload the context every time a file is modified, so we use this class merely
// to inform cubent that the change has occurred, and tell cubent to reload the file before making
// any changes to it. This fixes an issue with diff editing, where cubent was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to cubent via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of cubent, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	async setupFileWatcher(filePath: string) {
		...
		watcher.onDidChange(() => {
			if (this.recentlyEditedByRoo.has(filePath)) {
				this.recentlyEditedByRoo.delete(filePath) // This was an edit by cubent, no need to inform cubent
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform cubent
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to cubent via a tool, mention, or edit.
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			switch (source) {
				...
				// roo_edited: cubent has edited the file
				case "roo_edited":
				...
				// read_tool/file_mentioned: cubent has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.roo_read_date = now
					break
	...
	// Marks a file as edited by cubent to prevent false positives in file watchers
