...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".cubent", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .cubent/system-prompt-[mode slug]
...
/**
 * Ensures the .cubent directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".cubent")
