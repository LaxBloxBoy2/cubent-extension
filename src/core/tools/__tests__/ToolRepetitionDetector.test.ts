...
import type { ToolName } from "@cubent/types"
...
jest.mock("../../../i18n", () => ({
	t: jest.fn((key, options) => {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `cubent appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy.`
		}
		return key
	}),
}))
