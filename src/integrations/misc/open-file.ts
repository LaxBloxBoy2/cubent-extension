...
export async function openFile(filePath: string, options: OpenFileOptions = {}) {
	try {
		...
		if (filePath.startsWith("./")) {
			// Handle ./ prefix - resolve relative to workspace root
			fullPath = workspaceRoot ? path.join(workspaceRoot, filePath.slice(2)) : filePath
		} else if (path.isAbsolute(filePath)) {
			// Already an absolute path
			fullPath = filePath
		} else {
			// Relative path without ./ prefix - resolve relative to workspace root
